...
beta_mod <- function(beta, n_ref, ldrho, varz=NULL, maf=NULL, vary){
  ...
  # if ncovar>0 then sigma should adjust by covar
  Sigma <- abs(vary-t(bzy)%*%var_zz%*%bzy)
  zz <- ginv(var_zz)  
...
MR_RIVER<-function(bzx, bzx_se, bzx_pval, bzy, bzy_se,  snpid, 
                   bzx_maf, varz_x=NULL, MAF=TRUE, var=FALSE,
                   ldrho, varx, n_ref, eps = 1e-6, 
                   gwas_thresh=1,ld_r2_thresh=1){
  if(var==TRUE){
    mrdata <- data.frame(snpid,bzx,bzx_se,bzx_pval,bzy,bzy_se,varz_x)
  }
  
  if(MAF==TRUE){
    mrdata <- data.frame(snpid,bzx,bzx_se,bzx_pval,bzy,bzy_se,bzx_maf)
  }
  
  # GWAS prune based on bzx_pval
  mrdata <- mrdata[mrdata$bzx_pval<=gwas_thresh,]
  rho <- ldrho[mrdata$snpid,mrdata$SNP]
  ...
  if(MAF==TRUE){
    tmp <- beta_mod(beta=mrdata$bzx,n_ref=n_ref,ldrho=rho,
                    varz=NULL,maf=mrdata$bzx_maf,vary=varx)
    
  }
  if(var==TRUE){
    tmp <- beta_mod(beta=mrdata$bzx, n_ref=n_ref, ldrho=rho,
                    varz=mrdata$varz_x, vary=varx, maf=NULL)
  }
